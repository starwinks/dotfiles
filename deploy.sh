...
find_existing_app() {
    ...
    case "$app" in
        conda)
            ...
            ;;
        zsh)
            candidates+=("${DATA_DIR}/apps/zsh-env/bin/zsh")
            candidate="$(find_command zsh)"
            [[ -n "$candidate" ]] && candidates+=("$candidate")
            ;;
        ohmyzsh)
            candidates+=(
...
install_zsh() {
    ...
    [[ ! -e "$launcher" && ! -L "$launcher" ]] || die "zsh launcher already exists: $launcher"
    ln -s -- "$target/bin/zsh" "$launcher"
    APP_PATHS[zsh]="$launcher"
}
...
write_paths_file() {
    ...
    local local_profile=none local_module
    for local_module in exports.local.zsh func.local.zsh alias.local.zsh; do
        if [[ -r "$REPO_DIR/zsh/modules/$local_module" ]]; then
            local_profile=repo
            break
        fi
    done
    {
        ...
        write_shell_assignment DOTFILES_LOCAL_PROFILE "$local_profile"
...
main() {
    ...
    if ((DRY_RUN)); then
        ...
        info "data: $DATA_DIR"
        info "machine paths: $TARGET_HOME/.config/dotfiles/paths.env"
        info "Current shell was not modified; apply it with: source $TARGET_HOME/.config/dotfiles/paths.env"
        info "managed zsh: ${APP_PATHS[zsh]} -l"
